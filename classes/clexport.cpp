...
clexport::clexport(clcontext *context_, std::string name_, std::string kname_, parameters par_, clbuffer *src, int d_, int r_, bool img_, bool ts_, bool vp_) {
  context = context_;
  par = par_;
  ...
  be_export_image = new clbuffer(context, "be_" + s_name, kx, ky, kz);

  ke_render = new clkernel(context, par, s_kname, "./classes/parameters.h", "./kernels/k_header.cl");

  ke_render->bind("b_source",   b_source);
  ke_render->bind("b_target",   be_export_image);
...
void clexport::write_files(int it) {
  ...
  for (int j = 0; j < kj; j++) {
    for (int i = 0; i < ki; i++) {
      r = be_export_image->get(i,j,0,0);
      g = be_export_image->get(i,j,0,1);
      b = be_export_image->get(i,j,0,2);
      a = be_export_image->get(i,j,0,3);
